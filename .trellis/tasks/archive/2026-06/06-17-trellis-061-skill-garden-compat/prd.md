...
将 flower-trellis 捆绑的 `@mindfoldhq/trellis` 升级到 0.6.2，并修正 skill-garden 0.6 强化内容中仍引用 Trellis 0.6.0 `Phase 3.1 Quality verification` 的覆盖语义。升级后，skill-garden 的工作流覆盖、路由 skill、同步快照和当前安装副本都应使用 Trellis 0.6.1+ 的语义：3.1 已折叠进 2.2 final pass 与 3.4 spec-sync preamble；0.6.2 进一步补齐 `/continue` 对已删除 3.1 的路由修复。

## Requirements

- `package.json` / `package-lock.json` 使用 `@mindfoldhq/trellis@0.6.2`。
- `.trellis/.version` 更新到 `0.6.2`，并通过 Trellis 0.6.2 模板更新自动可更新文件。
...
- `trellis-continue` 中 `check passed → 3.1` 的上游残留通过 Trellis 0.6.2 更新修复。
- 不移除 finish-work release inference / `trellis-release` 强化能力。
- 不修改 Trellis 上游源码或 npm 全局安装目录。

## Acceptance Criteria

- [ ] `npm view @mindfoldhq/trellis@0.6.2 version` 显示 0.6.2，`package.json` / lockfile 依赖也固定为 0.6.2。
- [ ] `trellis update --dry-run` 不再显示项目版本低于 0.6.2；如仍有 modified-by-you 项，均为本仓需要保留的本地配置或 skill-garden 注入。
- [ ] `.agents/skills/trellis-continue/SKILL.md` 与 `.claude/commands/trellis/continue.md` 不再将 `check passed` 路由到 3.1，而是到 Phase 3.3 → 3.4。
...
- Trellis 0.6.2 是承接 0.6.1 workflow 简化的文档 / 模板修复，重点修 `/continue` 路由到已删除 3.1 的残留，不需要 `--migrate`。
- 用户明确关心 skill-garden 强化内容和 Trellis 0.6.1 语义之间的真实冲突，而不是所有上游模板副本差异。
- `trellis-continue` 的 3.1 恢复指向已由上游 0.6.2 修复，本任务可无损更新。
