...
3. 汇总 missing target、optional skip 与 required error。
...
5. 返回 changed/unchanged/skipped、target、backup note、result 与 provenance。
...
2. 同时加载 Skill-Garden catalog 与 Flower platform catalog。
3. 对所有 required Patch 全量 preflight，再统一 apply。
4. 复制 skill、script、Flower asset 和已启用 common skill。
5. 全装时按旧 manifest 精确清理 stale path。
6. 所有步骤成功后写包含 Patch provenance 的 manifest。

0.6 禁止再调用 `injectWorkflow()`、skill/hook override injector 或 Codex/Claude tweak。`injectWorkflow()` 与旧 tweak 只服务 0.5/old。

Skill-Garden 独立安装器顺序同样固定：版本/目标解析 → `apply-trellis-patches.py` → 复制 helper/common/Trellis 资产。不得在 `install.sh` 内维护第二份 0.6 文件修改实现。
...
两者必须保持 schema、Bundle 选择、路径安全、selector、baseline、marker 迁移、required/optional、preflight、首次备份、changed-only、catalog hash 和 provenance 语义一致。协议变更必须同时增加 JS/Python 测试。
...
至少覆盖三种 operation、`content.sources`、Core selector、平台 adapter、legacy migration、required/optional、target policy、普通文件 create 拒绝、新建父目录与备份软链逃逸、首次备份、非目标内容保留、全量 preflight 零写入、manifest 最后写、真实 catalog 预检和二次 dogfood 幂等。
