...
OIDC Trusted Publishing,自动带 provenance、无长期 token。
...
| `.github/workflows/release.yml` | `on: push: tags: ['v*']`;`npm publish`(OIDC)+ `gh release create` |

完整发布动作:`npm run release` → 检查 diff → `git push --follow-tags origin main`。
...
- push `vX.Y.Z` tag 触发 CI;CI checkout **不拉 submodule**(发布只依赖已提交的 `enhancements/` 快照)。
- CI 步骤:`npm i -g npm@latest` → `npm publish`(OIDC,不设 NODE_AUTH_TOKEN)→ `extract-changelog` → `gh release create --notes-file`。
