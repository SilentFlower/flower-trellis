...
1. API-only 的 Issue、Note、Discussion Reply 等动作使用 `--live --live-mode api`，只核对 `glab` 显式 host、当前用户和目标 Project 可读；不要求本地 repo，也不把 Developer 作为通用 API 写入门槛。
2. API 模式返回的角色只表示平台已公开的 direct / inherited membership 事实，不证明写权限。平台没有可靠只读能力字段时，由冻结后的目标端点响应判定：成功后按真实 ID 全文回读；确定性 403 时确认未创建对象并停止；结果不明时不重发。
3. 分支、push、MR 等动作使用 `--live --live-mode repo`，回读 `origin`、当前分支、ahead/behind、staged/unstaged/untracked、近期提交和提交姓名/邮箱。
4. Developer 是发起个人分支、push 和 MR 的权限底线；权限不足时不扩大 token scope、不改权限、不绕过受保护分支。
5. remote 只输出协议、host 形态和项目路径，不回显 userinfo 或完整凭据 URL。公司新 GitLab 的域名形态为推荐入口；已登记的 IP 形态只做识别和分流，不自行修改 remote。
6. 目录与分支不匹配、ahead/behind 异常或存在来源不明修改时保留现场，不切分支、不清理、不改写历史。
...
6. 一次授权可以覆盖同一冻结清单内的连续低影响动作及其回读，前提是唯一目标、动作清单、完整载荷或确定性生成规则和停止条件都已由本人确认。执行中不为每个命令、字段或只读回读重复询问；目标对象、动作种类、正文或字段等实质载荷、source/target、身份、权限或风险边界任一变化时停止并重新确认。结果不明不视为原授权仍可重试。
7. Issue Link、Discussion Reply 等关系型动作必须同时冻结两端真实 ID 和关系类型。
8. 正文含跨 Project 引用时，冻结前扫描裸 `!<iid>` / `#<iid>`。这些引用永远在当前 Project 解析——指向本仓对象时有效，指向其它 Project 时静默重定向（同号对象已存在）或降为纯文本（不存在时看不出异常）。跨 Project 引用必须使用 `group/project!<iid>`、`group/project#<iid>` 或完整 URL。判例："`rd-goal` checkpoint Note 用裸 `!3` 指代电商协作仓 MR，实际指向 `rd-goal` 自己的 `MR !3`"本身就是此类问题——当前仓已有的同号对象会接管引用。
9. 含跨 Project 引用的正文，全文 round-trip 通过后还须渲染回读。提交已写入内容的 markdown 渲染，带目标 `project` 参数，逐条核对链接的实际 Project 与期望目标是否一致。全文一致不能证明链接指向正确——这是两层独立性不同的检查。盲区包括 Note 头「覆盖」字段、章节标题和引用他人原文的引号内。
...
证据文件使用 `schema_version: 1` 和非空 `checks` 数组；每项包含唯一 `id`、`kind` 与 `input`。仓内正向样例见 `tests/skills/xhgj-gitlab-collaboration/fixtures/operation-evidence-v0.3.0.json`。
...
- MR 创建或更新、assignee / reviewer 设置，以及 pipeline 和 mergeability 的只读回读；不合并。
...
Issue / Task 的 assignee 继续只在用户明确要求某人承接工作项时设置，不为通知、抄送或“可能会处理”使用该字段。

进入 Project 默认分支的 MR 必须在冻结载荷中明确一名 assignee，表示人工评审、推进与收口责任人；责任人不明确时停止创建或更新，不自行猜测。GitLab reviewer 字段属于 `your taste`，只在用户或团队明确选择使用 review queue / 状态投影时设置，留空不构成缺失责任或合并阻断。人的 reviewer 职责独立存在，不得被任一平台字段替代。

MR assignee 只证明责任已指派，不证明 review 已完成；完成状态以 Discussion、明确评审结论、验证记录和人工合并结果回读为证据。author 与 assignee 为同一人时属于 self-review / self-merge 风险信号，必须按变更风险补充非作者评审，或留下显式例外、依据和接受的风险。写后分别回读 assignee 与 reviewer；预期 reviewer 留空时也要确认实际为空，任一字段与冻结载荷不一致都不能报告整组设置成功。
...
- MR description 至少覆盖背景、改动、验证、风险与关联 Issue。关闭 squash 和 auto-merge；目标为 Project 默认分支时明确一名 assignee，reviewer 字段按需留空；合并权归人。
- assignee、reviewer 字段和人的 review 完成证据分别验收。assignee 不等于 review 通过；author 与 assignee 相同时补非作者 review 或显式例外留痕。
