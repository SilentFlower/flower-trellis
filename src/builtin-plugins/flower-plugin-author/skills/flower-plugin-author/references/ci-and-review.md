...
```bash
flower-trellis plugin validate .flower-plugin/marketplace.json --subject marketplace --checkout-map .flower-plugin/checkouts.json --ci --json
```
...
`review.required=true` 时，CI 必须运行随模板分发的 `verify-integration-review.mjs`。该脚本要求 `.flower-plugin/integration-review.json` 使用以下结构并绑定本次 Marketplace digest：
