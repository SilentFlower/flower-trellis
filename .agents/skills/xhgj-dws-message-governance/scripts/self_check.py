#!/usr/bin/env python3
"""Offline validation and upgrade-difference check for the DWS governance Skill."""
...
REQUIRED_PREFLIGHT_CASES = {
    ...
    "source-unavailable",
    "payload-drift",
    "named-mention-mismatch",
    "manual-at-all",
    "multiline-cmd-shim",
    "markdown-numbering",
    "mention-bootstrap-valid",
    "ordinary-mention-without-evidence",
    "mention-bootstrap-business-action",
    "mention-bootstrap-missing-authorization",
    "mention-bootstrap-extra-recipient",
}
FORBIDDEN_TEXT = {
    "v0.1.5-" + "offline-preview",
    "valid_" + "until",
    "offline_" + "authorization",
    "rca-" + "ops",
    "openConversation" + "Id",
    "ticket-ops-management" + "@",
...
def validate_usage_evidence(
    ...
    for name in ("agent_review", "maintainer_review"):
        ...
        require_string(item["status"], f"review.{name}.status")
...
def validate_cases(path: Path) -> Tuple[int, int]:
    data = require_exact_keys(
        load_json_compatible(path), {"schema_version", "routing_cases", "preflight_cases"}, "cases"
    )
    ...
    for index, item in enumerate(routing_cases):
        ...
        require_string_array(
            case["expected_reads"], f"routing_cases[{index}].expected_reads"
        )
    ...
    for path in sorted(root.rglob("*")):
        ...
        for forbidden in FORBIDDEN_TEXT:
            if forbidden in text:
                ...
                raise CheckError(f"forbidden legacy or private locator text in {relative}: {forbidden}")
...
def validate_skill(root: Path = ROOT) -> Dict[str, Any]:
    ...
    if aliases != ["dws-message-governance"] or any(
    ...
    if contract["source"] != "migration:xhgj-dws-message-governance":
        raise CheckError("contract.yaml.source must identify the A7 migration")
    ...
    if usage["status"] != "executed" or usage["review_status"] != "pending-maintainer-review":
        raise CheckError("P1 real usage must remain executed and pending Maintainer review")
    ...
    source_markers = (
        ...
        "docs/standards/dingtalk-message-governance.md",
        "docs/playbooks/dws-message-delivery.md",
        "required-source",
        "fail closed",
    )
    ...
    plan_contract = (root / "references" / "message-plan-contract.md").read_text(
        encoding="utf-8-sig"
    )
    plan_markers = (
        "payload_sha256",
        "scripts/preflight.py",
        "mention-bootstrap-ready",
        "controlled_recipient_test_confirmed",
        "eligible_for_ordinary_send",
        "--bootstrap-result",
    )
    if not all(marker in plan_contract for marker in plan_markers):
        raise CheckError("message-plan-contract.md is missing deterministic gate markers")
    openai = (root / "agents" / "openai.yaml").read_text(encoding="utf-8-sig")
    if "$xhgj-dws-message-governance" not in openai:
        raise CheckError("agents/openai.yaml default prompt must name the Skill")
    compile((root / "scripts" / "preflight.py").read_text(encoding="utf-8-sig"), "preflight.py", "exec")
...
def main() -> int:
    ...
    if args.json:
        ...
        print(
            "xhgj-dws-message-governance self-check passed: "
