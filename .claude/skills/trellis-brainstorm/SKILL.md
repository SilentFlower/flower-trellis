...
For non-trivial project work, run the Project Knowledge Discovery entry owned by `trellis-before-dev` before choosing an approach; use `spec_router.py` to locate project-local SOPs and specs, then return here with the relevant evidence.
