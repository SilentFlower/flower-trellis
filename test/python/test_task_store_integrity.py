...
class TaskStoreIntegrityTest(unittest.TestCase):
    ...
    def test_create_initializes_pending_closeout(self) -> None:
        """新任务从 pending Close 状态开始，不借用物理位置表达语义。"""
        with self.loaded_store() as module:
            self.assertEqual(module.cmd_create(self.create_args("closeout")), 0)

        task_dir = next((self.root / ".trellis/tasks").glob("*-closeout"))
        data = json.loads((task_dir / "task.json").read_text(encoding="utf-8"))
        self.assertEqual(
            data["closeout"],
            {"status": "pending", "closedAt": None, "blockers": []},
        )
    ...
    def test_add_subtask_second_write_failure_restores_both_files(self) -> None:
        ...
        with self.loaded_store() as module:
            ...
            with mock.patch.object(module, "write_json", side_effect=controlled_write):
                result = module.cmd_add_subtask(Namespace(parent_dir="parent", child_dir="child"))
